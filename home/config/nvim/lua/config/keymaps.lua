...
-- Custom Enter and Drag Select Mode (from utils/mappings.lua)
local previous_signcolumn = nil

local function custom_enter()
...
local function drag_select_mode()
  if vim.o.signcolumn ~= "no" then
    previous_signcolumn = vim.o.signcolumn
    vim.opt.signcolumn = "no"
    vim.opt.number = false
  else
    if previous_signcolumn then
      vim.opt.signcolumn = previous_signcolumn
    end
    vim.opt.number = true
  end
end
...
local function setup()
    -- Custom Mappings
    nmap('<CR>', custom_enter)
    nmap('<F9>', drag_select_mode)
